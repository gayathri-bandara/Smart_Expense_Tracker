...
export default function EditTransactionScreen({ route, navigation }) {
  ...
  const [image, setImage] = useState(
    transaction.receiptImage ? `${getUploadsBaseUrl()}/uploads/${transaction.receiptImage}` : null
  );
  ...
  const handleSubmit = async () => {
    if (!title.trim() || !amount.trim() || !category.trim() || !date.trim()) {
      showAlert("Missing fields", "Fill in title, amount, category, and date.");
      return;
    }
    ...
    try {
      ...
      console.log(err);
      showAlert("Error", "Could not update transaction. Check the backend is running.");
    } finally {
      setSubmitting(false);
    }
  };
